...
interface EstimateStore {
  ...
  recalculateAll: () => void;
  updateMetalDeckCost: (cost: number) => void;
  updateMetalDeckErection: (field: 'area' | 'pricePerSqft', value: number) => void;
}
...
export const useEstimateStore = create<EstimateStore>((set, get) => ({
  ...
  recalculateAll: () => {
    set((state) => {
      ...
      // Calculate reference values for structural steel
      if (structuralSteel.totalTons > 0) {
        structuralSteel.pricePerTon = Math.ceil(structuralSteel.totalCost / structuralSteel.totalTons);
      }
      if (structuralSteel.area > 0) {
        structuralSteel.pricePerSqft = Math.ceil(structuralSteel.totalCost / structuralSteel.area);
      }

      // Calculate hours per ton
      const totalHours = structuralSteel.shopLabour.reduce((sum, item) => sum + item.hours, 0);
      if (structuralSteel.totalTons > 0) {
        structuralSteel.hoursPerTon = Math.round((totalHours / structuralSteel.totalTons) * 100) / 100;
      }

      // Update erection tons to include OWSJ weight
      structuralSteel.erectionFreight.tons = structuralSteel.totalTons + (structuralSteel.owsj.weight / 2000);

      // Calculate metal deck total cost
      if (metalDeck.visible) {
        metalDeck.erection.totalCost = Math.ceil(metalDeck.erection.area * metalDeck.erection.pricePerSqft);
        metalDeck.totalCost = Math.ceil(metalDeck.deckCost + metalDeck.erection.totalCost);
      }
  ...
  updateMetalDeckCost: (cost) => {
    set((state) => {
      const updatedMetalDeck = {
        ...state.currentEstimate.metalDeck,
        deckCost: cost
      };
  ...
  updateMetalDeckErection: (field, value) => {
    set((state) => {
      const updatedMetalDeck = {
        ...state.currentEstimate.metalDeck,
        erection: {
          ...state.currentEstimate.metalDeck.erection,
          [field]: value
        }
      };
